...
public class MyZoomImageView extends ImageView implements OnScaleGestureListener, OnTouchListener,
    ...
    private void checkBorderAndCenterWhenScale()
    {
        ...
        Log.e("bibi", "deltaX = " + deltaX + " , deltaY = " + deltaY);
    ...
    public void onGlobalLayout()
    {
        if (once)
        {
            ...
            if (d == null)
                return;
            Log.e("bibi", d.getIntrinsicWidth() + " , " + d.getIntrinsicHeight());
            int width = getWidth();
            int height = getHeight();
            // 拿到图片的宽和高
            ...
            if (dw > width && dh > height)
            {
                scale = Math.min((float) width * 1.0f / (float) dw, (float) height / (float) dh
                        * 1.0f);
            }
            initScale = scale;
            // 图片移动至屏幕中心
